...
<?foreach ($arResult['ITEMS'] as $key => $arItem) {
    ...
    if ($arItem['PROPERTIES']['OLD_PRICE']['VALUE'] > 0 && $arItem['PROPERTIES']['OLD_PRICE']['ACTIVE'] == 'Y') {
        ...
        if ($oldPrice > ($maxPriceValue / $arItem['CATALOG_MEASURE_RATIO'])) {
            $maxPriceValue = $oldPrice * $arItem['CATALOG_MEASURE_RATIO'];
        }
    }
    if ($minPriceValue <= 0) {
        if ((float)$arItem['PROPERTIES']['MIN_PRICE']['VALUE'] > 0 && 
            (float)$arItem['PROPERTIES']['MIN_PRICE']['VALUE'] < (float)$maxPriceValue) {
            $minPriceValue = $arItem['PROPERTIES']['MIN_PRICE']['VALUE'];
        }
    }
    $discount = ($maxPriceValue > 0 && $minPriceValue > 0) ? ($maxPriceValue - $minPriceValue) : 0;
    if ($arItem['CATALOG_MEASURE_RATIO'] === 1) {
        $arItem['MIN_PRICE']['DISCOUNT_VALUE'] = $minPriceValue;
        $arItem['MIN_PRICE']['VALUE'] = $maxPriceValue;
        $arItem['MIN_PRICE']['ECONOMY'] = $discount;
    } else {
        $arItem['MIN_PRICE']['DISCOUNT_VALUE'] = $minPriceValue / $arItem['CATALOG_MEASURE_RATIO'];
        $arItem['MIN_PRICE']['VALUE'] = $maxPriceValue / $arItem['CATALOG_MEASURE_RATIO'];
        $arItem['MIN_PRICE']['ECONOMY'] = $discount / $arItem['CATALOG_MEASURE_RATIO'];
    }
    $itemType = array();
    $itemClass = '';
