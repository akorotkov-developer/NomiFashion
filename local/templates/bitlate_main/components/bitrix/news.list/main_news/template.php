...
<?if (!empty($arResult['ITEMS'])):?>
    <div class="main-news">
        ...
        <div class="owl-carousel main-news-carousel is-lazy" data-slideout-ignore>
            <?foreach ($arResult['ITEMS'] as $index => $arItem):
                ...
                if ($pic !== false || $arParams['DISPLAY_PICTURE'] == "N"):
                    ...
                    <a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                        <?if ($pic !== false):?>
                            <img src="<?=NLApparelshopUtils::getLoadSrc($pic['src'])?>" data-src="<?=$pic['src']?>" alt="<?=$title?>" class="<?=NLApparelshopUtils::getLoadOwlClass()?>">
                        <?endif;?>
