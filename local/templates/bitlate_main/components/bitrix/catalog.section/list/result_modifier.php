...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
            ...
            if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
                    ...
                    if (!$arOffer['OWNER_PICT'])
					{
                        ...
                        if ($arOffer['PREVIEW_PICTURE']['ID'] > 0) {
                            $pic = NLApparelshopUtils::getResizeImg($arOffer["PREVIEW_PICTURE"]['ID'], $arSizePreview);
                        }
                        if ($pic === false) {
                ...
                if ($arItem['PRODUCT_PREVIEW']['ID'] > 0) {
                    $pic = NLApparelshopUtils::getResizeImg($arItem["PRODUCT_PREVIEW"]['ID'], $arSizePreview);
                }
                if ($pic === false) {
                    $pic['src'] = $strEmptyPreview;
                }
        ...
        $rating = floatval($arItem['PROPERTIES']['rating']['VALUE']);
        $vote_count = intval($arItem['PROPERTIES']['vote_count']['VALUE']);

        if (empty($rating))
            $rating = floatval($arItem['PROPERTIES']['DEFAULT_RATING']['VALUE']);

        if ($rating > 5)
            $rating = 5;

        $arItem['RATING'] = $rating;
        $arItem['VOTE_COUNT'] = $vote_count;
        $arItem['SHOWED_PROPERTIES'] = array();
        foreach(CIBlockSectionPropertyLink::GetArray($arParams['IBLOCK_ID'], $arItem['IBLOCK_SECTION_ID']) as $PID => $arLink)
        {
            $arItem['SHOWED_PROPERTIES'][] = $PID;
        }
