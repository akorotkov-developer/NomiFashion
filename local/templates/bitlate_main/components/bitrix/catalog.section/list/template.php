...
if (!empty($arResult['ITEMS']))
{
    ...
    $isPriceComposite = COption::GetOptionString("bitlate.apparelshop", "NL_CATALOG_PRICE_COMPOSITE", false, SITE_ID);
...
<?foreach ($arResult['ITEMS'] as $key => $arItem) {
    ...
    $typePict = NLApparelshopUtils::getSkuPictType($arItem);
    if ($typePict == 'square' || $typePict == 'dropdown') {
        $isShowPreviewPict = NLApparelshopUtils::isShowPreviewPict($arItem);
    }?>
    <div class="product-list-item table-container h100pc w100pc" id="<? echo $strMainID; ?>">
        <div class="table-item small-3 large-4 xxlarge-3 h100pc column-preview text-center relative">
            ...
            <div class="img-wrap">
                <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"><img id="<? echo $arItemIDs['PICT']; ?>" src="<?=NLApparelshopUtils::getLoadSrc($pic['src'])?>" data-src="<?=$pic['src']?>" class="thumbnail lazy" alt="<? echo $productTitle; ?>"></a>
            </div>
        ...
        <div class="table-item small-9 large-8 xxlarge-9 h100pc">
                ...
                <div class="row">
                    <div class="column column-info-block small-6 medium-4 large-6 xxlarge-4">
                        <div class="rating" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
                            ...
                            <meta itemprop="ratingCount" content="<?=$arItem['VOTE_COUNT']?>">
            ...
            <div class="column large-6 xxlarge-4 relative column-action">
                <div class="price-block" id="<? echo $arItemIDs['PRICE']; ?>_block">
                    ...
                    <?endif;?>
                        <?if ($isPriceMulty && count($arItem['PRICES']) > 1 && !in_array('prodday', $itemType) && !in_array('action', $itemType)):?>
                        ...
                        <?if (!empty($arItem['OFFERS_PROP'])) {
                            ...
                            '</div>';
                                        ...
                                        elseif ('PICT' == $arProp['SHOW_MODE'])
                                        {
                                            $templateRow .= '<div class="inline-block-container show-for-large" id="#ITEM#_prop_'.$arProp['ID'].'_cont">'.
                            '<div class="product-info-caption inline-block-item">'.htmlspecialcharsex($arProp['NAME']).':&nbsp;</div>'.
                            '<div class="product-info-option '.(($typePict == 'round') ? 'color' : 'image').' inline-block-item"><fieldset class="inline-block-container" id="#ITEM#_prop_'.$arProp['ID'].'_list">';
                                            foreach ($arProp['VALUES'] as $arOneValue)
                                            {
                                                if (in_array($arOneValue['ID'], $arPropIds)) {
                                                    ...
                                                    if ($typePict == 'round') {
                                                        ...
                                                        $templateRow .= '<img src="'.NLApparelshopUtils::getLoadSrc($pisSrc).'" data-src="'.$pisSrc.'" alt="'.$arOneValue['NAME'].'" class="lazy">';
                                                    }
                            ...
                            foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer) {
                                ...
                                if ($isPriceMulty && count($arItem['OFFERS'][$keyOffer]['PRICES']) > 1) {
                                    $iPrice = 0;
