...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($arElement = $rsElements->Fetch())
	{
        ...
        if (is_array($arElement["PRICES"]) && count($arElement["PRICES"]) <= 0) {
            ...
            if (is_array($arOffers[0]['PRICES']) && count($arOffers[0]['PRICES']) > 0) {
                $arElement["PRICES"] = $arOffers[0]['PRICES'];
            }
        }
			...
			if(array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"]))
			{
				...
				if ($arParams["SHOW_PREVIEW"] == "Y")
				{
					if ($arElement["PREVIEW_PICTURE"] > 0)
						$arElement["PICTURE"] = NLApparelshopUtils::getResizeImg($arElement["PREVIEW_PICTURE"], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL);
					elseif ($arElement["DETAIL_PICTURE"] > 0)
						$arElement["PICTURE"] = NLApparelshopUtils::getResizeImg($arElement["DETAIL_PICTURE"], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL);
				}
			}
			break;
	}
