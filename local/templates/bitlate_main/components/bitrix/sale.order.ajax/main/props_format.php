...
if (!function_exists("PrintPropsForm")) {
    function PrintPropsForm($arSource = array(), $locationTemplate = ".default") {
        ...
        if (!empty($arSource)) {
            ...
            $iAddress = null;
            $iLocation = null;
            foreach ($arSource as $i => $arProperties) {
                if ($arProperties['CODE'] == 'ADDRESS') {
                    $iAddress = $i;
                } elseif ($arProperties['CODE'] == 'CITY') {
                    $iLocation = $i;
                }
                if ($arProperties["CODE"] == "ZIP" && $arProperties["TYPE"] == "TEXT" && $arProperties["REQUIRED"] == "Y") {
                    $rules .= "\"{$arProperties["FIELD_NAME"]}\"" . ': {"required": true, "minlength": 6}, ';
                } elseif ($arProperties["CODE"] == "ZIP" && $arProperties["TYPE"] == "TEXT") {
                    $rules .= "\"{$arProperties["FIELD_NAME"]}\"" . ': {"minlength": 6}, ';
                } elseif ($arProperties["IS_EMAIL"] == "Y" && $arProperties["TYPE"] == "TEXT" && $arProperties["REQUIRED"] == "Y") {
                    $rules .= "\"{$arProperties["FIELD_NAME"]}\"" . ': {"required": true, "email": true}, ';
                } elseif ($arProperties["IS_EMAIL"] == "Y" && $arProperties["TYPE"] == "TEXT") {
                    $rules .= "\"{$arProperties["FIELD_NAME"]}\"" . ': {"email": true}, ';
                } elseif ($arProperties["REQUIRED"] == "Y" && ($arProperties["TYPE"] == "TEXT" || $arProperties["TYPE"] == "TEXTAREA")) {
                    $rules .= "\"{$arProperties["FIELD_NAME"]}\"" . ': {"required": true}, ';
                }
            }
            if ($iAddress !== null && $iLocation !== null) {
                $tmpLocation = $arSource[$iLocation];
                $arSource[$iLocation] = $arSource[$iAddress];
                $arSource[$iAddress] = $tmpLocation;
            }
            foreach ($arSource as $i => $arProperties) {
                $subClass = '';
                ...
                $prevClass = $subClass;?>
                <div class="columns<?=$subClass?>" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
                    <?if ($arProperties["TYPE"] == "CHECKBOX") {?>
                        <input type="hidden" name="<?=$arProperties["FIELD_NAME"]?>" value="">
                        <input type="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" value="Y" id="<?=$arProperties["FIELD_NAME"]?>" class="show-for-sr"<?if ($arProperties["CHECKED"]=="Y") echo " checked=\"checked\"";?>>
                        <label for="subscribe-news"><?=$arProperties["NAME"]?></label>
                    ...
                    <?} elseif ($arProperties["TYPE"] == "TEXT") {
                        ...
                        if ($arProperties["CODE"] == "PHONE") {
                            $inputClass = 'phone';
                        } elseif ($arProperties["CODE"] == "ZIP") {
                            $inputClass = 'zip';
                        }?>
                        <input type="text" class="<?=$inputClass?>" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="<?=$arProperties["NAME"]?>">
                        <?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
                            <div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
                        <?endif?>
                    ...
                    <?} elseif ($arProperties["TYPE"] == "TEXTAREA") {
                        ...
                        <textarea rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="<?=$arProperties["NAME"]?>"><?=$arProperties["VALUE"]?></textarea>
                        <?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
                            <div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
                        <?endif?>
                    ...
                    <?} elseif ($arProperties["TYPE"] == "FILE") {?>
                        <div><?=$arProperties["NAME"]?></div>
                        <?if($arProperties["VALUE"]>0):?>
                            <?$arFile = \CFile::GetFileArray($arProperties["VALUE"]);?>
                            <span id="<?=$arProperties["FIELD_NAME"]?>_span">
                                <a download="<?=$arFile["ORIGINAL_NAME"]?>" href="<?=CFile::GetFileSRC($arFile)?>">
                            ...
                            <label for=""><input type="file" size="<?=$arProperties["SIZE1"]?>" value="" name="ORDER_PROP_<?=$arProperties["ID"]?>_ALT[0]" id="ORDER_PROP_<?=$arProperties["ID"]?>_ALT" onclick="document.getElementById('<?=$arProperties["FIELD_NAME"]?>_span').remove(); document.getElementById('ORDER_PROP_<?=$arProperties["ID"]?>_ALT').name = 'ORDER_PROP_<?=$arProperties["ID"]?>[0]';"></label>
                        <?else:?>
            ...
            <?if ($_POST["is_ajax_post"] != "Y"):?>
                <script>
                    $(document).ready(function(){
                        initValidateOrder("#ORDER_FORM", <?=$rules?>);
            ...
            <?else:?>
                <span id="prop-rules-object" style="display: none;"><?=$rules?></script>
            <?endif;?>
        <?}
    }
}
